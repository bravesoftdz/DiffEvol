...
var
   ...
   pc7_TB,pc8_TB,pc9_TB, P31_TB: real; //глобальная переменная, принимающая значение при изменении бегунка
   ...
   pc9_TB_L,SolarMod_TB_L:boolean;  //логическая переменная, которая будет показывать основной процедуре, что данные изменились

   k1:integer;
   p24:real; //добавить описание, если сработает!
   //переменные со значениями прироста для графиков
   years_array, years_ini:integer100; //годы из файла с реальными данными
   gr_ini:real100; //значения из файла с реальными данными
   gr_norm, ini_norm,gr_middle,another_gr_middle:real100; //значения из программы
...
procedure noof(path:string;
               var pathcli,filecli:string);
var
p,id4:integer;
begin
 p:=1;
...
if (year mod 100  = 0 ) {and (year mod 400<>0)} then
 begin
 if year mod 4 = 0 then Leap_year:=366 else //showmessage('âèñîêîñíûé' ) else
...
begin
 ...
 begin
  ...
  END;
 END;
 if check51=true then check20:=false;
 end;
end;

procedure INPCLI(climfile:string;
                 P:  array30;
                 var temp, prec: array366;
                 var M:integer;
                 var ERR:integer;
                 var NU:integer);
var
...
readln(f);
{$I+}
If IOresult <>0 then error:=error+1;
...
For i:=1 to Leap_year do
 ...
 if prec[i]=-9999 then prec[i]:=0
 end;
for i:=1 to Leap_year do  temp[I]:=Temp[i]/10;
for i:=1 to Leap_year do  prec[I]:=prec[i]/10;
 M1:=1;
 M2:=Leap_year;
 ii:=Leap_year;
 ...
 begin
  ...
  IF(PREC[I]<=0.0) then PREC[I]:=0;
 END;
end;

procedure FILTER (F: array366;
                  N:integer;
                  TFL: string;
                  var F_1:array366);
var
...
procedure DAYLENGTH(LAT: real;NU: integer;M:integer;
                    var DL: array366 ); {входные\выходные}
var
Pi: real;
i:integer;
fi:real;
...
// if lat>54 then lat:=54;
 ...
 begin
   ...
   if cos_hs[i]>1 then cos_hs[i]:=1;
   if cos_hs[i]<-1 then cos_hs[i]:=-0.9;
...
begin
IF(x <= a1) or (x>= a4) THEN  TFUNC:=0.0;
IF(x > a1) and (x<= a2) THEN   TFUNC:=(x-a1)/(a2-a1);
if (x > a2) and (x<= a3) THEN   TFUNC:= 1;
if (x > a3) and (x< a4) THEN   TFUNC:=(a4-x)/(a4-a3);
IF (TFUNC<= 0) then TFUNC:=0.0;
if (TFUNC> 1) then TFUNC:=1.0;
end;
...
begin
 ...
 begin
  ...
  Sc:=B[19]-K[6]*sk;
  b6:=sk*B[16]+sc;
  IF(b6>B[18]) then b6:=B[18];
  IF(b6<B[19]) then b6:=B[19];
  VP:=b6*j*b[5]+B[7];
 End
 ELSE
  VP:=B[6]*j*b[5]+B[7];
...
procedure grrt50(TEM: array366;
                 ...
                 KC: real10;
                 dl: array366;
                 filex,result_file:string;
                 year:integer; //год для которого производятся расчеты
                 grrt_check:integer;
                 n_beg,n_end:integer;
                 total_precin:real;
                 pinim_in:real; //влажность почвы в первый день с момента начала оттаивания почвы
                 var gr: array366;
                 var sm: array366;
                 var gr_middle:real;
                 var MB:INTEGER;
                 var Beg_gr1,end_gr1,Beg_gr2,end_gr2:integer; //начало сезона роста по суммарной температуре
                 var pinim:real; // влажность почвы когда блок оттаивания не включен
                 var pinim_out:real; //влажность почвы в последий день с момента начала замерзания почвы
                 var growth_rate:real; //критическая скорость в день когда суммарная температура больше критической
                 var total_precout:real; //суммарная влажность с начала года по sm_dep - день
                 var begdep,enddep:integer; //номер дня когда глубина корней не отрицательная
                 var temtmin,temtbeg:real; //temtmin суммарная температура в день когда tem[i]>tmin; temtbeg температура когда total_temp>tbeg
...
begin
 ...
 begin
  ...
  begin
   ...
   MM:=i+10{K[10]};
   i:=i+1;
   if sumtemp<p[7] then sumtemp:=0.0;
  END;
  for i:=1 to (MM) do dep[i]:=0;
  for i:=MM+1 to M  do
  begin
   dep[i]:=dep[i-1]+P[8]*TEM[i-1]*EXP((-1)*P[9]*dep[i-1]);
   IF(dep[i]<=0) then dep[i]:=0;
   if (Dep[i]>p[13]) then dep[i]:=P[13];
 ...
 //суммарная влажность 06.08.15
  ...
  enddep:=0;
  for i:=1 to m do
  if (begdep=0) and (total_temp[i]>p[22]) and(tem[i]>p[1]) then begdep:=i;
  ...
  //обнуление
  for i:=1 to 366 do moisture[i]:=0;
  for i:=1 to M do
  ...
  moisture[1]:=p[10];
  if moisture[1]< p[10] then moisture[1]:=p[10];
  snow[1]:=p[23]; // начальное значение объема снежного покрова
  for i:=2 to m do snow[i]:=0;
  ...
  for i:=1 to (M-1) do
  begin
   TFUNC_pro(moisture[i],p[17],p[18],p[19],p[5],trw); //если убрать то значения не сохраняются
   trd:=1;
   if trw>1 then trw:=1;
   IF(k[1]=1) then trd:=dep[i]/P[13];
   sm_double[i]:=trw*trd;
...
//   if (i>(begdep+14)) and (vcr>gr_double[i]) and (vcr>gr_double[i-1]) and (enddep=0) then begin enddep:=i-1; End_gr1:=i-1; end;
   ...
   tr[i]:=gr_double[i]*P[14]*EXP(TEM[i]*P[15]);      //транспирация воды кроной дерева
   //gr_double[i]
   IF(K[4]=0)THEN
   begin
    xmelt:=0;
 ...
 {available precipitation}
   ...
   IF(smoist[i]<=0) then smoist[i]:=0;
   //08.07
   if k[1]=1 then
   begin
    if (dep[i+1]>0) and (dep[i]>0) then
    moisture[i+1]:=smoist[i]/dep[i+1]
    else moisture[i+1]:=moisture[i];
   end;
   if k[1]=0 then moisture[i+1]:=smoist[i]/dep[i+1];

  IF(moisture[i+1]>=P[5]) then moisture[i+1]:=P[5];
  IF(moisture[i+1]<=P[17]) then moisture[i+1]:=P[17]; //минимальное занчение не может быть меньше W1
  end;
//  IF(k[1]=1)and (dep[m]=0) then moisture[m]:=moisture[m-1];
  TFUNC_pro(moisture[m],p[17],p[18],p[19],p[5],sm_double[m]);
  if sm_double[m]>1 then sm_double[m]:=1;
  if (m=366) or (m=365) then sm_double[m]:=sm_double[m-1];
  ...
  for i:=1 to m do cumulitive_rate[i]:=0;
  for i:=1 to m do
  for j:=1 to i do
  ...
  begin
   FI:=NU+i;
   WRITE(f1,year:4,(NU+i):4,' ',TEM[i]:8:2,PRE[i]:8:2,moisture[i]:8:4,k6[i]:8:2,tr[i]:6:2,dep[i]:8:2,snow[i]:6:2,gr_double[i]:8:4);
   writeln(f1,' ',sm_double[i]:8:4,' ',grtemp[i]:8:2,' ',dl[i]:6:2, cumulitive_rate[i]:8:2,Total_temp[i]:8:2, P[22]:10:0);
  end;
  i_index:=0;
 ...
 //начало и конец сезона роста находится в цикле
  ...
  begin
   ...
   if (i>(Beg_gr1+14)) and (vcr>gr_double[i]) and (vcr>gr_double[i-1]) and (End_gr1=0) then begin End_gr1:=i-1; end;
   //bimodal growth season
   if (beg_gr2=0) and (end_gr1<>0) and (i>(end_gr1+14)) {and (gr_double[i]>vcr)} and (total_temp[i]>p[22]) and (tem[i]>p[1]) then begin Beg_gr2:=i; end;
   // end_gr1 - end of first growth period; growth rate higher than critical rate, and total temperatures higher
   if (beg_gr2=M) or (beg_gr2+14>=M) then beg_gr2:=0;
   if (Beg_gr2<>0) and (i>(Beg_gr2+10)) and (vcr>gr_double[i]) and (vcr>gr_double[i-1]) and (End_gr2=0) then begin End_gr2:=i-1; end;
   if (Beg_gr2<>0) and (End_gr2=0)  and (i=M-1) then End_gr2:=Beg_gr2+14;
   If (Beg_gr2<>0) and (Beg_gr2+14>M) then end_gr2:=M;
   if (Beg_gr2<>0) then enddep:=Beg_gr2;
  end;
  {while (Beg_gr1=0)  do
 ...
 { total_precout:=0;
  ...
  for i:=1 to m do   //если интегральная скорость больше критической увеличенной!
  ...
  if i_index<>0 then gr_middle:=gr_middle{/growth_season} else //деление на количество элементов
  begin
   gr_middle:=PC1_TB;
  end;
  growth_rate:=gr_double[Beg_gr1]; //интегральная скорость роста в начало сезона роста
  closefile(f1);
end;
//------------------ GRRT50----------------------
...
Procedure CAMB50(VEXT: array366;
                 NU:integer;
                 FILEX,file_ini:string;
                 NVC,KTIME:integer;
                 B:real20;
                 K:real10;
                 year:integer;
                 check_c,check_p:integer;
                 var NRING:integer;
                 var NR:rmax_int;
                 var TD:rmax_real;
                 var SI:rmax_real);
...
begin  //7
 check130:=false;
 N0:=N1;
 IF(K0>RMAX) then check131:=true;
 if check131=false then
 begin  //8
  T:=T+T0;
  IT:=T;
  IF(IT<=KTIME)then check140:=true;
 end;  //8
 if check140=false then check131:=true;
 {130}
 ...
 begin   //9
  ...
  begin  //10
  check120:=false;
  if (IT<K[9]-NU)then check130:=true;
  if check130=false then
  begin  //11
   IF(SI[J]< 0.1) then check155:=true;
   if check155=false then
   begin  //12
    if (DIV1[J]='N') then check150:=true;
    ...
    begin  //14
     if (V<VS) then check150:=true  //cell in rest - next j
     else
     begin //15
    ...
    begin  //18
     AT[J]:=AT[J]+T0;
     SI[J]:=SI[J]+V0*T0;
     IF SI[J]<=SIM THEN Check150:=true;
    end;  //18
...
// NRING:=NRING-N1;
 ...
 begin
  ...
  end else begin assignfile(F2,FP); append(f2); end;
  for I:=1 to N0 DO
  begin
   IDD:=0;
   IF(DIV1[I]='Y') then IDD:=1;
   z:=0;
   IF(I>=2)AND (KP[I]<>0) then z:=AT[I]/KP[I];
   WRITEln(f2,year,' ',I,'   ',KP[I],'    ',RT[I]:2:2,'     ',AT[I]:2:2,'    ',z:2:2,'    ',SI[I]:2:2,'   ',IDD,'   ',(size[i]*45.0+5.0):2:2);    {check format}
  END;
  CLOSE(f2)
 END;
 begin
...
begin
if a<b then max1:=b else max1:=a;
end;

procedure RTMR (year:integer;
                n:integer;
                fname:string;
                ne:integer;
...
begin
 ...
 begin
  ...
  { S1:=0.1;} s1:=1;
   ...
   IF(N=0) then showmessage('N=0 should stop') else {dialog window b delphi!!}
   begin
    V:=V/N;
    i:=-1;
    s:=0;
    ...
    begin
     ...
     begin
      ...
      if (s< V*j) then  check110:=true;
      if check110=false then t[j]:= S1*I;
     end;
    end;
    for i:=n downto 2 do
    begin
     t[i]:=t[i]-t[i-1];
     IF(t[i]=0) then t[i]:=S1;
    END;
   end;
   if (t[1]<>0) then
   begin
    ...
    for i:=2 to n do
    begin
     if (t[i]=0) then check_TI:=true;
     if check_TI=false then v:=v+1.0/t[1];
    end;
...
//-------reading---------------
procedure reading(namefile:string; begyear,endyear:integer; var year:integer100;var gr_result:real100);
var
f:textfile;
i,j,k:integer;
year1:integer100;
...
begin
 ...
 //условие для деления на среднее значение от предыдущих расчетов
  ...
  if mean_rates = 1 then gr_y:=mean_in;
 for i:=1 to n do
...
begin
 ...
 y_1:=0;
 numerator:=0;
 for i:=1 to n do  x_mid:=x[i]+x_mid;
 x_mid:=x_mid/n;
 for i:=1 to n do  y_mid:=y[i]+y_mid;
 y_mid:=y_mid/n;
 x_1:=0;
 y_1:=0;
 for i:=1 to n do
...
Begin
 ...
 Begin
  ...
  for j:=I to (i+10) do
  sum:=sum+tem[j];
  if (sum>Tbeg) and (n1=0) then n1:=I+9;
 end;
 if n1<346 then  i:=n1+20;
 if (n1>366) or (n1<0) then n1:=366;
 i:=n1;
 While n2=0  do
 Begin
  I:=i+1;
  ...
  for j:=I to (i+10) do
  sum:=sum+tem[j];
  if (sum<Tbeg) and (n2=0)  then n2:=I;
 end;
 if n2>=366 then n2:=366;
end;
...
Begin
 ...
 begin
  for j:=I to (i+14) do
  if (tem[j]>0.0) then sum:=sum+1;
  if sum=15 then n1:=j else sum:=0;
 end;
 sum:=0;
 for i:=n1 to (m-14) do
 if n2=0 then
 begin
  for j:=i to (i+14) do
  if (tem[j]<0.0) then sum:=sum+1;
  if sum=15 then n2:=i else sum:=0;
 end;
 if n2=0 then n2:=m;
{ for i:=1 to m do
...
grrt_check,check_c,check_s,check_p,check_t:integer; //счетчик действий в процедурах для зпписи файлов
JJ:integer;
...
file_exist:boolean;    //переменная для проверки пропуска года климатики
...
begdep,enddep:integer; //начало и конец сезона оттаивания/замерзания почвы
Dep_beg,Dep_end:array [1..100] of integer;
temtmin,temtbeg:array [1..100] of real; //temtmin суммарная температура в день когда tem[i]>tmin; temtbeg температура когда total_temp>tbeg
...
begin
 ...
 {$I+}
 If IOresult <>0 then
 Application.MessageBox(PChar('Error reading! File of Parametres'+filename_1 +' check file please'),
 'Внимание',1);
 ...
 if p1_TB_l=True then p[1]:=p1_TB;
 if p2_TB_l=True then P[2]:=p2_TB;
 if p3_TB_l=True then P[3]:=p3_TB;
 if p4_TB_l=True then P[4]:=p4_TB;
 if p5_TB_l=True then P[5]:=p5_TB;
 if p7_TB_l=True then P[7]:=p7_TB;
 if p8_TB_l=True then P[8]:=p8_TB;
 if p9_TB_l=True then P[9]:=p9_TB;
 if P10_TB_l=True then P[10]:=P10_TB;
 if p12_TB_l=True then P[12]:=p12_TB;
 if p13_TB_l=True then P[13]:=p13_TB;
 if p14_TB_l=True then P[14]:=p14_TB;
 if p15_TB_l=True then P[15]:=p15_TB;
 if p16_TB_l=True then P[16]:=p16_TB;
 if p17_TB_l=True then P[17]:=p17_TB;
 if p18_TB_l=True then P[18]:=p18_TB;
 if p19_TB_l=True then P[19]:=p19_TB;
 if p22_TB_l=True then P[22]:=p22_TB;
 if PC1_TB_l=true then PC[1]:=PC1_TB;
 if PC6_TB_l=true then PC[6]:=PC6_TB;
 if PC7_TB_l=true then PC[7]:=PC7_TB;
 if PC8_TB_l=true then PC[8]:=PC8_TB;
 if PC9_TB_l=true then PC[9]:=PC9_TB;
 If SolarMod_TB_l=true then P[31]:=P31_TB;
 k[1]:=k1;
 //k[4]:=k1;
 //
  p1_TB:=p[1];
 ...
 //проверка, если минимальное значение больше максимального
  if p[1]>p[2] then   p[2]:=p[1]+1;
  if p[2]>p[3] then   p[3]:=p[2]+1;           // исправить в самих бегунках
  if p[3]>p[4] then   p[4]:=p[3]+1;

 //сохранение данных для файла
  for i:=1 to 31 do P_res[i]:=p[i];
  for i:=1 to 20 do pc_res[i]:=pc[i] ;
  for i:=1 to 20 do K_res[i]:=k[i];
  for i:=1 to 10 do KC_res[i]:=kc[i] ;
  for i:=1 to 10 do PS_res[i]:=ps[i] ;
  for i:=1 to 10 do KS_res[i]:=ks[i] ;
 ...
 begin
  ...
  result_file_rate:=result_path+areaname+' rate '+inttostr(begyear)+' - ' + inttostr(endyear)+'.dat';
...
{  IF (FILT='A') or (FILT='L') or (FILT='V') then
  ...
  years[Year_number]:=year; //массиву лет присваиваем значение года
  GRRT50(TEM,PRE,Nu,M,k,p,pc,kc,dl,climfile,result_file_rate,year,grrt_check,n_beg,z_end,total_precin,pinim_in,gr,sm,
  gr_middle[Year_number],T_beg,Beg_gr1,End_gr1,Beg_gr2,End_gr2,Pinim,pinim_out,growth_rate,total_precout,begdep,enddep,temmin,tembeg,daymin);
...
{------CREATE FILE FOR OUTPUT INFORMATION-----}
 ...
 for i:=1 to jj1 do
 begin
  writeln(f5,years[i]:6,another_gr_middle[i]:8:4,gr_ini[I]:8:4,gr_norm[i]:8:4,ini_norm[i]:8:4,daytmin[i]:6, BGROWTH1[I]:8,EGROWTH1[I]:8,BGROWTH2[I]:6,EGROWTH2[I]:6{, p[22]:8:1,GT[i]:6,EGT[i]:6,
  Dep_beg[i]:8,Dep_end[i]:8,piniin[i]:8:2,piniout[i]:8:2,BGW[i]:6,EGW[i]:6},temtmin[i]:8:2,temtbeg[i]:8:2,vcr[i]:8:2);
 end;
{_______________}
 {for i:=1 to jj1 do
